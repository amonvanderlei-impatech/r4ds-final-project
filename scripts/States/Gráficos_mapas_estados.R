...
municipios <- read.csv2("Porcentagem_municipios_ANO.csv")
estados <- read.csv2("Porcentagem_estados_ANO.csv")
...
plot1 <- ggplot(
  ...
  labs(
    ...
    subtitle = "Nota média no ENEM ANO do município em função da renda média dos alunos do município",
    x = "Renda média (salários mínimos)",
    y = "Nota média",
    color = "Unidade Federativa",
    caption = "Fonte: Microdados ENEM ANO"
  )+
  theme_bw()+
  theme(
...
print(plot1)
ggsave(filename = "Dados/Plots/Grafico_Nota_Renda_Cor_Estado_ANO.png", plot=plot1)
rm(plot1)
...
plot2 <- ggplot(
  ...
  labs(
    ...
    subtitle = "Nota média no ENEM ANO da UF em função da renda média dos alunos da UF",
    x = "Renda média (salários mínimos)",
    y = "Nota média",
    color = "Unidade Federativa",
    caption = "Fonte: Microdados ENEM ANO"
...
print(plot2)
ggsave(filename = "Dados/Plots/Grafico_Nota_Renda_Estados_ANO.png", plot=plot2)
rm(plot2)
...
print(plot3)
ggsave(filename = "Dados/Plots/Grafico_Nota_Renda_Agrupados_Estados_ANO.png", plot=plot3)
rm(plot3)
...
for (i in UFs) {
  ...
  ggsave(filename = paste0("Dados/Plots/Individuais_Estado/Grafico_Nota_Renda_", i, "_ANO.png"), plot = grafico)
...
write.csv2(coeficientes_estados, "Dados/Coeficientes_Estados_ANO.csv")
...
plot4 <- ggplot() +
  ...
  labs(
    title = "RENDA MÉDIA",
    subtitle = "Visualização da renda média dos estudantes por unidade federativa (salários mínimos)",
    fill = "Renda Média",
    x = NULL, y = NULL,
    caption = "Fonte: Microdados ENEM ANO"
  ) +
...
print(plot4)
ggsave(filename = "Dados/Plots/Mapa_Renda_Estados_ANO.png", plot=plot4)
rm(plot4)
...
plot5 <- ggplot() +
  ...
  labs(
    title = "NOTA MÉDIA",
    subtitle = "Visualização da nota média no ENEM ANO dos estudantes por unidade federativa",
    fill = "Nota Média",
    x = NULL, y = NULL,
    caption = "Fonte: Microdados ENEM ANO"
  ) +
...
print(plot5)
ggsave(filename = "Dados/Plots/Mapa_Nota_Estados_ANO.png", plot=plot5)
rm(plot5)
...
plot6 <- ggplot() +
  ...
  labs(
    ...
    subtitle = "Visualização da razão da nota média no ENEM ANO pela Renda média dos estudantes por unidade federativa",
    fill = "Razão",
    x = NULL, y = NULL,
    caption = "Fonte: Microdados ENEM ANO"
  ) +
...
print(plot6)
ggsave(filename = "Dados/Plots/Mapa_Nota_Div_Renda_Estados_ANO.png", plot=plot6)
rm(plot6)

write.csv2(razao_nota_renda, "Dados/Nota_Div_Renda_Estados_ANO.csv")
